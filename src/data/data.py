...
def load_s3_tif(s3_name):
    print('--Loading s3 file "{}"'.format(s3_name))
    s3 = rasterio.open(s3_name).read().astype('float32') # (21, rows, cols) --> s3 has 21 bands
    s3 = np.moveaxis(s3, source=0, destination=-1) # (rows, cols, 21)
    print('--Product pre-processing...')
    s3 = np.clip(s3, 0, 1)
    return s3

def load_s2_tif(s2_name, RESIZE_SHAPE=None):
    print('--Loading s2 file "{}"'.format(s2_name))
    s2 = rasterio.open(s2_name).read().astype('uint16') # (12, rows, cols) --> s2 has 13 bands but one is remove for the atmospheric correction process
...
def load_s3_npz(s3_name, NORM=True):
    print('--Loading s3 file "{}"'.format(s3_name))
    s3 = np.load(s3_name)['arr_0'] # (rows, cols, 21)
    print('--Product pre-processing...')
...
def load_s2_npz(s2_name, RESIZE_SHAPE=None, NORM=True):
    print('--Loading s2 file "{}"'.format(s2_name))
    s2 = np.load(s2_name)['arr_0'] # (rows, cols, 13)
    print('--Product pre-processing...')
    s2 = np.clip(s2.astype(np.float32)/10000, 0, 1) # recovering the original reflectances of s2 (see https://forum.step.esa.int/t/dn-to-reflectance/15763/8, https://gis.stackexchange.com/questions/233874/what-is-the-range-of-values-of-sentinel-2-level-2a-images)
    if NORM:
        print('--Normalizing...')
...
def load_data_products(s3_names, s2_names, s3_gt_names, patch_size, step, max_patch_size, downscale_s2=True, use_single_channel=True, NORM=True):
    '''
    El método devuelve tensores con las imagenes preparadas para alimentar a la red,
    los tensores tienen las dimensiones (patches, bands, rows_patch, cols_patch),
    es decir, las imagenes dividas en parches, con las bandas en el eje 1. Además
    devuelve las dimensiones de las imagenes originales (s3 y s2).
    '''
    ...
    if len(set([len(s3_names), len(s3_names), len(s3_gt_names)]))>1:
    ...
    for i in range(len(s3_names)):
        ...
        #s3 = load_s3_npz(s3_names[i], NORM)
        s3 = load_s3_mat(s3_names[i], NORM)
        print('--Loading {}...'.format(s2_names[i]))

        # If downscale_s2 is True, we resize the s2 product to the s3 size (5490, 5490) -> (366, 366)
        if downscale_s2:
            #s2 = load_s2_npz(s2_names[i], (s3.shape[0],s3.shape[1]), NORM)
            s2 = load_s2_mat(s2_names[i], (s3.shape[0],s3.shape[1]), NORM)
        else:
            #s2 = load_s2_npz(s2_names[i], None, NORM)
            s2 = load_s2_mat(s2_names[i], None , NORM)
        print('--Loading {}...'.format(s3_gt_names[i]))
        #s3_gt = load_s3_npz(s3_gt_names[i], NORM)
        s3_gt = load_s3gt_mat(s3_gt_names[i], NORM)
        if use_single_channel:
            print('--Selecting a single channel for registration...')
            #El expand dims solo sirve para que las imagenes pasen de (366, 366) a (366, 366, 1)
    ...
    # CREO QUE SE HACEN PASOS INNECESARIOS, AL HACER all_s3.append(x) all_s3 PASA A SER [x]
    # Y AL HACER all_s3 = np.concatenate(all_s3, axis=0) all_s3 PASA A SER x, POR QUÉ NO HACER
    # PROPONGO SUSTITUIR LAS LÍNEAS; 167, 168, 194, 201, 202, 203, 208, 209 Y 210 POR:
    # all_s3, all_s2, all_s3_gt = extract_patches(s3, s2, s3_gt, patch_size, step, max_patch_size)
    # if all_s3.ndim ==4: all_s3 = np.moveaxis(all_s3, -1, 1) 
    # if all_s2.ndim ==4: all_s2 = np.moveaxis(all_s2, -1, 1)
    # if all_s3_gt.ndim ==4: all_s3_gt = np.moveaxis(all_s3_gt, -1, 1)
...
def imadjust(img): 
    #p1, p2 = np.percentile(img, (2.5, 97.7))
